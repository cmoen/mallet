...
public class HierarchicalLDA {
    ...
    public void samplePath(int doc, int iteration) {
		...
		ObjectDoubleOpenHashMap<NCRPNode> nodeWeights = 
			new ObjectDoubleOpenHashMap<NCRPNode>();
		...
		IntIntOpenHashMap[] typeCounts = new IntIntOpenHashMap[numLevels];

		int[] docLevels;

		for (level = 0; level < numLevels; level++) {
			typeCounts[level] = new IntIntOpenHashMap();
		}
		...
		for (level = 1; level < numLevels; level++) {  // Skip the root...
			int[] types = typeCounts[level].keys().toArray();
			int totalTokens = 0;
    ...
    public void calculateWordLikelihood(ObjectDoubleOpenHashMap<NCRPNode> nodeWeights,
										NCRPNode node, double weight, 
										IntIntOpenHashMap[] typeCounts, double[] newTopicWeights,
