...
public class TopicModelDiagnostics {
	...
	public void collectDocumentStatistics () {
		...
		IntOpenHashSet[] topicTopWordIndices = new IntOpenHashSet[numTopics];
    ...
    IntOpenHashSet[] docTopicWordIndices = new IntOpenHashSet[numTopics];
		...
		for (int topic = 0; topic < numTopics; topic++) {
      IntOpenHashSet wordIndices = new IntOpenHashSet();
			...
			docTopicWordIndices[topic] = new IntOpenHashSet();
		}
		...
		for (TopicAssignment document: model.getData()) {
            ...
            IntOpenHashSet supportedWords = docTopicWordIndices[topic];
