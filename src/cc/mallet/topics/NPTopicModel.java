...
public class NPTopicModel implements Serializable {
	...
	protected IntIntOpenHashMap tokensPerTopic; // indexed by <topic index>

	// The number of documents that contain at least one
	//  token with a given topic.
	protected IntIntOpenHashMap docsPerTopic;
	...
	public void addInstances (InstanceList training, int initialTopics) {
		...
		for (int type=0; type < numTypes; type++) {
			typeTopicCounts[type] = new IntIntOpenHashMap();
		}
      ...
      IntIntOpenHashMap topicCounts = new IntIntOpenHashMap();
	...
	public String topWords (int numWords) {
		...
		for (int topic: docsPerTopic.keys().toArray()) {
			for (int type = 0; type < numTypes; type++) {
