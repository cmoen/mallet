...
public class WeightedTopicModel implements Serializable {
	...
	protected IntDoubleOpenHashMap[] typeTypeWeights;
	...
	public void readTypeTypeWeights (File weightsFile) throws Exception {
		...
		typeTypeWeights = new IntDoubleOpenHashMap[numTypes];

		logger.info("num types: " + numTypes);

		for (int type = 0; type < numTypes; type++) {
			typeTypeWeights[type] = new IntDoubleOpenHashMap();
	...
	protected void sampleTopicsForOneDoc (FeatureSequence tokenSequence,
      ...
      IntDoubleOpenHashMap typeFactors = typeTypeWeights[type];
			int[] connectedTypes = typeFactors.keys().toArray();
