...
public class LDAHyper implements Serializable {
	...
	protected IntIntOpenHashMap[] typeTopicCounts; // indexed by <feature index, topic index>
    ...
    IntIntOpenHashMap currentTypeTopicCounts;
		...
		for (int token = 0; token < docLen; token++) {
			...
			topicCounts = currentTypeTopicCounts.values().toArray();
    ...
    IntIntOpenHashMap currentTypeTopicCounts;
		...
		//		populate topic counts
		IntIntOpenHashMap localTopicCounts = new IntIntOpenHashMap();
		...
		for (int topic: localTopicCounts.keys().toArray()) {
		...
		for (int position = 0; position < docLength; position++) {
			...
			topicTermValues = currentTypeTopicCounts.values().toArray();
...
//			Make sure it actually gets set
			...
			if (sample < topicTermMass) {
				...
				if (sample < topicBetaMass) {
					...
					topicTermIndices = localTopicCounts.keys().toArray();
					topicTermValues = localTopicCounts.values().toArray();
	...
	public void topicXMLReportPhrases (PrintStream out, int numWords) {
		...
		ObjectIntOpenHashMap<String>[] phrases = new ObjectIntOpenHashMap[numTopics];
		Alphabet alphabet = this.getAlphabet();
		
		// Get counts of phrases
		for (int ti = 0; ti < numTopics; ti++)
			phrases[ti] = new ObjectIntOpenHashMap<String>();
...
//					phrases[prevtopic].increment(sbs);
	...
	*/
	...
	private void readObject (ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		typeTopicCounts = new IntIntOpenHashMap[numTypes];
		for (int fi = 0; fi < numTypes; fi++)
			typeTopicCounts[fi] = (IntIntOpenHashMap) in.readObject();
		tokensPerTopic = new int[numTopics];
		for (int ti = 0; ti < numTopics; ti++)
	...
	public double modelLogLikelihood() {
		...
		for (int type=0; type < numTypes; type++) {
			int[] usedTopics = typeTopicCounts[type].keys().toArray();
