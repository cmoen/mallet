...
public class FeatureWindow extends Pipe implements java.io.Serializable {
    ...
    public Instance pipe (Instance carrier) {
        ...
        ObjectDoubleOpenHashMap[] original = new ObjectDoubleOpenHashMap[seq.size()];
        for (int i=0; i<seq.size(); i++) {
            Token t = seq.get(i);
            original[i] = new ObjectDoubleOpenHashMap();
        ...
        for (int i=0; i<original.length; i++) { // add to features of token i...
            for (int j = -1 * left; j <= right; j++) {
                ...
                Token t = seq.get(i);
                Object[] features = original[index].keys().toArray();
