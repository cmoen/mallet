...
public abstract class TwoLabelGEConstraints implements GEConstraint {
  ...
  protected IntIntOpenHashMap constraintsMap;
  ...
  public TwoLabelGEConstraints() {
    ...
    this.constraintsMap = new IntIntOpenHashMap();
    this.map = null;
    this.cache = new IntArrayList();
  }
  
  protected TwoLabelGEConstraints(ArrayList<TwoLabelGEConstraint> constraintsList, IntIntOpenHashMap constraintsMap, StateLabelMap map) {
  ...
  public BitSet preProcess(InstanceList data) {
    ...
    for (Instance instance : data) {
      ...
      for (int ip = 1; ip < fvs.size(); ip++) {
        for (int fi : constraintsMap.keys().toArray()) {
