...
public class OneLabelL2PRConstraints implements PRConstraint {
  ...
  protected IntObjectOpenHashMap<OneLabelPRConstraint> constraints;
  // maps between input feature indices and constraint indices
  protected IntIntOpenHashMap constraintIndices;
  ...
  public OneLabelL2PRConstraints(boolean normalized) {
    this.constraints = new IntObjectOpenHashMap<OneLabelPRConstraint>();
    this.constraintIndices = new IntIntOpenHashMap();
    this.cache = new IntArrayList();
    this.normalized = normalized;
  }
  
  protected OneLabelL2PRConstraints(IntObjectOpenHashMap<OneLabelPRConstraint> constraints,
                                    IntIntOpenHashMap constraintIndices, StateLabelMap map, boolean normalized) {
    this.constraints = new IntObjectOpenHashMap<OneLabelPRConstraint>();
    for (int key : constraints.keys().toArray()) {
  ...
  public void getExpectations(double[] expectations) {
    ...
    for (int fi : constraintIndices.keys().toArray()) {
