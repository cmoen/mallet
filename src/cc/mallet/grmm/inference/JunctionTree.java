...
public class JunctionTree extends Tree {
  ...
  private IntObjectOpenHashMap sepsets;
  private Factor[] cpfs;
  ...
  public JunctionTree(int size)
  {
    ...
    numNodes = size;
    sepsets = new IntObjectOpenHashMap();
  ...
  void clearCPFs()
  {
    ...
    Iterator<IntObjectCursor> it = sepsets.iterator();
    while (it.hasNext()) {
      IntObjectCursor cursor = it.next();
      Sepset sepset = (Sepset) cursor.value;
  ...
  public Set sepsetPotentials()
  {
    ...
    while (it.hasNext()) {
      IntObjectCursor cursor = it.next();
      Factor ptl = ((Sepset) cursor.value).ptl;
      set.add(ptl);
    }
